...
public class JeuDeGomokuFactory implements JeuDePlateauFactory {
    ...
    public JeuDeGomoku CreerPartieHumainVSAleatoire(ArrayList<Coup> situation) {
        if (situation == null) {
            ...
            return j;
        } else {/*JeuDeGomoku j = new JeuDeGomoku(2 == situation.get(situation.size() - 1).getId(),
...
//        return j;
        ...
        JeuDeGomoku j = new JeuDeGomoku(new JoueurAleatoire(p.getHistorique().get(0).getId()),
                new JoueurAleatoire(p.getHistorique().get(1).getId()), 4, p);
